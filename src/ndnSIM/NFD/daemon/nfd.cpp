...
// It is necessary to explicitly define the destructor, because some member
// variables (e.g., unique_ptr<Forwarder>) are forward-declared, but implicitly
// declared destructor requires complete types for all members when
// instantiated.
Nfd::~Nfd() = default;
...
void Nfd::initialize() {
  ...
  m_netmon->onNetworkStateChanged.connect([this] {
    ...
    m_reloadConfigEvent = getScheduler().schedule(5_s, [this] {
      NFD_LOG_INFO(
          "Network change detected, reloading face section of the config "
          "file...");
...
static inline void ignoreRibAndLogSections(const std::string& filename,
  ...
  if(sectionName == "rib" || sectionName == "log") {
    ...
    // missing NFD section
    ConfigFile::throwErrorOnUnknownSection(filename, sectionName, section,
                                           isDryRun);
  }
}

void Nfd::initializeManagement() {
  std::tie(m_internalFace, m_internalClientFace) =
      face::makeInternalFace(m_keyChain);
  m_faceTable->addReserved(m_internalFace, face::FACEID_INTERNAL_FACE);

  m_dispatcher =
      make_unique<ndn::mgmt::Dispatcher>(*m_internalClientFace, m_keyChain);
  m_authenticator = CommandAuthenticator::create();

  m_forwarderStatusManager =
      make_unique<ForwarderStatusManager>(*m_forwarder, *m_dispatcher);
  m_faceManager =
      make_unique<FaceManager>(*m_faceSystem, *m_dispatcher, *m_authenticator);
  m_fibManager = make_unique<FibManager>(m_forwarder->getFib(), *m_faceTable,
                                         *m_dispatcher, *m_authenticator);
  m_csManager =
      make_unique<CsManager>(m_forwarder->getCs(), m_forwarder->getCounters(),
                             *m_dispatcher, *m_authenticator);
  m_strategyChoiceManager = make_unique<StrategyChoiceManager>(
      m_forwarder->getStrategyChoice(), *m_dispatcher, *m_authenticator);
...
void Nfd::reloadConfigFileFaceSection() {
  // reload only face_system section of the config file to re-initialize
  // multicast faces
  ConfigFile config(&ConfigFile::ignoreUnknownSection);
  m_faceSystem->setConfigFile(config);
