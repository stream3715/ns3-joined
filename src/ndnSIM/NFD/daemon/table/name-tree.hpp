...
/** \brief A common index structure for FIB, PIT, StrategyChoice, and
 * Measurements
 */
class NameTree : noncopyable {
 ...
 public:  // information
   ...
   * the creation of many NameTree entries, which could take very long time.
   * This constant limits the maximum number of name components in the name of a
 ...
 public:  // mutation
   ...
   * Fib::s_emptyEntry \note This overload is more efficient than `lookup(const
   * Name&)` in common cases.
   */
  Entry& lookup(const fib::Entry& fibEntry);

  /** \brief Equivalent to `lookup(pitEntry.getName(),
   * std::min(pitEntry.getName().size(), getMaxDepth()))` \param pitEntry a PIT
   * entry attached to this name tree \note This overload is more efficient than
   * `lookup(const Name&)` in common cases.
   */
   ...
   *  \note This overload is more efficient than `lookup(const Name&)` in common
   * cases.
   */
  Entry& lookup(const measurements::Entry& measurementsEntry);

  /** \brief Equivalent to `lookup(strategyChoiceEntry.getPrefix())`
   *  \param strategyChoiceEntry a StrategyChoice entry attached to this name
   * tree \note This overload is more efficient than `lookup(const Name&)` in
   * common cases.
   */
   ...
   *  \param canEraseAncestors whether ancestors should be deleted if they
   * become empty \return number of deleted entries \sa Entry::isEmpty() \post
   * If the entry is empty, it's deleted. If \p canEraseAncestors is true,
   *        ancestors of the entry are also deleted if they become empty.
   *  \note This function must be called after detaching a table entry from a
   * name tree entry, \note Existing iterators, except those pointing to deleted
 ...
 public:  // matching
   ...
   *  \tparam EntryT \c fib::Entry or \c measurements::Entry or \c
   * strategy_choice::Entry \note This overload is more efficient than
   *        `findLongestPrefixMatch(const Name&, const EntrySelector&)` in
   * common cases. \warning Undefined behavior may occur if \p tableEntry is not
  ...
  /** \brief Equivalent to `findLongestPrefixMatch(pitEntry.getName(),
   * entrySelector)` \note This overload is more efficient than
   *        `findLongestPrefixMatch(const Name&, const EntrySelector&)` in
   ...
   *  \warning If a name tree entry whose name is a prefix of \p name is
   * inserted during the enumeration, it may or may not be visited. If a name
 ...
 public:  // enumeration
   ...
   *  \endcode
   *  \note Iteration order is implementation-defined.
   *  \warning If a name tree entry is inserted or deleted during the
   * enumeration, it may cause the enumeration to skip entries or visit some
   * entries twice.
   */
   ...
   *  \endcode
   *  \note Iteration order is implementation-defined.
   *  \warning If a name tree entry under \p prefix is inserted or deleted
