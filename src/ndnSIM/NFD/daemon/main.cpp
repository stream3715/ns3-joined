 ...
 *
 *  NFD (main forwarding procedure) and RIB manager execute in two different
 * threads. Each thread has its own instances of global io_service and global
 * scheduler.
 *
 *  When either of the daemons fails, execution of non-failed daemon will be
 * terminated as well.  In other words, when NFD fails, RIB manager will be
 * terminated; when RIB manager fails, NFD will be terminated.
 */
class NfdRunner : noncopyable {
 public:
  explicit NfdRunner(const std::string& configFile)
    ...
    m_terminationSignalSet.async_wait(
        bind(&NfdRunner::terminate, this, _1, _2));

    m_reloadSignalSet.add(SIGHUP);
    m_reloadSignalSet.async_wait(bind(&NfdRunner::reload, this, _1, _2));
  }

  void initialize() { m_nfd.initialize(); }

  int run() {
    // Return value: a non-zero value is assigned when either NFD or RIB manager
    // (running in a separate thread) fails.
    ...
    // Mutex and conditional variable to implement synchronization between main
    // and RIB manager threads:
    // - to block main thread until RIB manager thread starts and initializes
    // ribIo (to allow
    //   stopping it later)
    std::mutex m;
    std::condition_variable cv;

    std::thread ribThread(
        [configFile = m_configFile, &retval, &ribIo, mainIo, &cv, &m] {
          {
    ...
    {
      // Wait to guarantee that ribIo is properly initialized, so it can be used
      // to terminate RIB manager thread.
 ...
 private:
  void terminate(const boost::system::error_code& error, int signalNo) {
    ...
    NFD_LOG_INFO("Caught signal " << signalNo << " (" << ::strsignal(signalNo)
                                  << "), exiting...");
...
int main(int argc, char** argv) {
  ...
  description.add_options()("help,h", "print this message and exit")(
  ...
  try {
    ...
    // Cannot use NFD_LOG_* macros here, because the logging subsystem is not
    // initialized yet at this point. Moreover, we don't want to clutter error
    // messages related to command-line parsing with timestamps and other
    // useless text added by the macros.
  ...
  const std::string boostBuildInfo = "with Boost version " +
  ...
  std::clog << "NFD version " << NFD_VERSION_BUILD_STRING << " starting\n"
            ...
            << boostBuildInfo << ", " << pcapBuildInfo << ", " << wsBuildInfo
