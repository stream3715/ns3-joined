...
using KoNDNStrategyTester = StrategyTester<KoNDNStrategy>;
NFD_REGISTER_STRATEGY(KoNDNStrategyTester);

BOOST_AUTO_TEST_SUITE(Fw)

class KoNDNStrategyFixture : public GlobalIoTimeFixture
{
protected:
  KoNDNStrategyFixture()
...
protected:
  ...
  KoNDNStrategyTester strategy{forwarder};
...
BOOST_FIXTURE_TEST_SUITE(TestKoNDNStrategy, KoNDNStrategyFixture)
...
BOOST_AUTO_TEST_CASE(Forward)
{
  ...
  const time::nanoseconds TICK = time::duration_cast<time::nanoseconds>(
    KoNDNStrategy::RETX_SUPPRESSION_INITIAL * 0.1);
  ...
  periodicalRetxFrom4();
  this->advanceClocks(TICK, KoNDNStrategy::RETX_SUPPRESSION_MAX * 16);
  retxFrom4Evt.cancel();
  ...
  for (int i = 0; i < 3; ++i) {
    this->advanceClocks(TICK, KoNDNStrategy::RETX_SUPPRESSION_MAX * 2);
...
BOOST_AUTO_TEST_SUITE_END() // TestKoNDNStrategy
